...
from synergia.lattice import Lattice, Lattice_element, Mad8_parser, element_format
...
class Lattice_cache:
    def __init__(self, file_name, line_name):
        ...
        if self.real_file:
            ...
            self.cache_file_name = os.path.join(self.cache_dir, cache_file_basename)
            self.cache = None
            self.max_index = 0
    ...
    def is_readable(self):
        ...
        if exists:
            readable = (
                self.cache[self._versioned_line_name()].hash_hex == self._get_hash_hex()
            )
        return readable
...
class Mad8_reader:
    ...
    def _expand_type(self, short):
        ...
        for long in Lattice_element.get_all_type_names():
            if long.find(short) == 0:
                if retval:
                    raise RuntimeError(
                        'Mad8_reader: ambiguous abbreviated type "'
                        + short
                        + '": matches "'
                        + retval
                        + '" and "'
            ...
            if islist(entry):
                ...
                if entry == "-":
                    ...
                    self._extract_elements(
                        self._reverse(subentry), ancestors, lattice, True
                    )
                    i += 2
                elif entry[len(entry) - 1] == "*":
    ...
    def _get_possibly_cached_lattice(
        self, line_name, filename=None, enable_cache_write=True, enable_cache_read=True
    ):
    ...
    def get_lattice(
        ...
        if not islist(line_names):
            return self._get_possibly_cached_lattice(
                line_names, filename, enable_cache_write, enable_cache_read
            )
        else:
            lattices = []
            for theline in line_names:
                lattices.append(
                    self._get_possibly_cached_lattice(
                        theline, filename, enable_cache_write, enable_cache_read
                    )
                )
            return lattices
