...
def eliminate_markers(lattice):
    ...
    for element in lattice.get_elements():
        if (element.get_type_name() == "marker") and (not no_simplify(element)):
            pass
        else:
...
def convert_monitors(lattice):
    ...
    for element in lattice.get_elements():
        if (
            (element.get_type_name() == "monitor")
            or (element.get_type_name() == "hmonitor")
            or (element.get_type_name() == "vmonitor")
        ) and (not no_simplify(element)):
...
def convert_magnets(lattice):
    ...
    new_lattice.set_reference_particle(lattice.get_reference_particle())
    for element in lattice.get_elements():
        if _is_magnet(element.get_type_name()) and (not no_simplify(element)):
            ...
            if has_nonzero_strength:
                ...
                new_element = synergia.lattice.Lattice_element(
                    "drift", element.get_name()
                )
                _copy_attributes_ancestors(element, new_element)
                new_lattice.append(new_element)
        else:
...
def convert_kickers(lattice):
    ...
    for element in lattice.get_elements():
        if (
            (element.get_type_name() == "hicker")
            or (element.get_type_name() == "vkicker")
        ) and (not no_simplify(element)):
            if element.get_double_attribute("kick") == 0.0:
                ...
                new_lattice.append(element)
        elif (element.get_type_name() == "kicker") and (not no_simplify(element)):
...
def combine_drifts(lattice):
    ...
    for element in lattice.get_elements():
        if last_element:
            ...
            if (element.get_type_name() == "drift") and (not no_simplify(element)):
...
def simplify_all(lattice):
    return combine_drifts(
        convert_kickers(convert_magnets(convert_monitors(eliminate_markers(lattice))))
    )
