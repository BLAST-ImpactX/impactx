...
def test_commxx():
    commxx = Commxx()
    assert commxx.rank() == MPI.COMM_WORLD.Get_rank()
    assert commxx.size() == MPI.COMM_WORLD.Get_size()


# def test_decompose_1d_raw():
#     procs = 4
#     num_per_proc = 5
#     offsets, counts = synergia.utils.decompose_1d_raw(procs, procs * num_per_proc)
#     expected_offset = 0
