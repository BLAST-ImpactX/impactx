...
Similarly to other beam dynamics codes, most elements are modeled by using an ideal, hard-edge approximation for the potentials and fields.  In particular, this assumes that the fields are independent of the longitudinal path-length coordinate :math:`s`.
However, ImpactX also supports several additional soft-edge element models.  The models include:

* **RFCavity** - soft-edge model of an RF cavity using on-axis field data
* **SoftSolenoid** - soft-edge model of a solenoid using on-axis field data
* **SoftQuadrupole** - soft-edge model of a solenoid using on-axis field gradient data

For these elements, the user may specify the on-axis field or field gradient in one of two forms:

* using tabulated on-axis data
