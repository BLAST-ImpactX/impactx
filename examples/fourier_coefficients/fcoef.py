...
def calculate_coefficients(data, ncoef):
    ...
    for j in range(ncoef):
        zz = zdata[0] - zmid
        cos_coeffs[j] += (-0.5 * edata[0] * math.cos((j) * 2 * pi * zz / zlen) * h) / zhalf
        sin_coeffs[j] += (-0.5 * edata[0] * math.sin((j) * 2 * pi * zz / zlen) * h) / zhalf
        zz = zdata[-1] - zmid
        cos_coeffs[j] -= (0.5 * edata[-1] * math.cos((j) * 2 * pi * zz / zlen) * h) / zhalf
        sin_coeffs[j] -= (0.5 * edata[-1] * math.sin((j) * 2 * pi * zz / zlen) * h) / zhalf
...
def write_coefficients(cos_coeffs, sin_coeffs, filename):
    ...
    with open(filename, 'w') as f:
        f.write(f"j  cos_coef[j]  sin_coef[j] \n")
    ...
    with open('onaxis_data.out', 'w') as f:
        for i, z in enumerate(zdata):
            ...
            for j in range(1, len(cos_coeffs)):
                tmpsum += cos_coeffs[j] * math.cos((j) * 2 * math.pi * zz / zlen) + sin_coeffs[j] * math.sin((j) * 2 * math.pi * zz / zlen)
                tmpsump += -(j) * 2 * math.pi * cos_coeffs[j] * math.sin((j) * 2 * math.pi * zz / zlen) / zlen + (j) * 2 * math.pi * sin_coeffs[j] * math.cos((j) * 2 * math.pi * zz / zlen) / zlen
                tmpsumpp += -((j) * 2 * math.pi / zlen)**2 * cos_coeffs[j] * math.cos((j) * 2 * math.pi * zz / zlen) - ((j) * 2 * math.pi / zlen)**2 * sin_coeffs[j] * math.sin((j) * 2 * math.pi * zz / zlen)
