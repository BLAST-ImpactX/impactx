...
initial distribution is a uniformly-populated cylinder of transverse radius rb = 2 mm with zero momentum spread.

The beam propagates in a drift with a vacuum chamber radius of R = 3.5 mm.
...
This induces a phase space correlation within the beam, such that px = k*x and py = k*y, similar to what would be induced by a space charge kick.

The beam remains cylindrical with zero emittance during its evolution in a 6 m drift.
In the absence of an aperture, the beam radius evolves as :math:`rb(s) = rb(1 + ks)`.
...
:math:`Q_s/Q_0 = \min{1,R^2/(rb^2(1+sk)^2)}`.
