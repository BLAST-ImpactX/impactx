...
void init_refparticle(py::module& m)
{
    py::class_<RefPart>(m, "RefPart")
        ...
        .def_readwrite("px", &RefPart::px, "momentum in x divided by m*c = beta_x*gamma [m*c]")
        .def_readwrite("py", &RefPart::py, "momentum in y divided by m*c = beta_y*gamma [m*c]")
        .def_readwrite("pz", &RefPart::pz, "momentum in z divided by m*c = beta_z*gamma [m*c]")
