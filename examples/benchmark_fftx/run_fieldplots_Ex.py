...
def run_and_plot(case, r, ax):
    ...
    case : string
      case name
    r : Float
      aspect ratio = sigma_z / sigma_perp:  range 0.1 to 10
    ax : matplotlib.axes
    ...
    sim.n_cell = [128, 128, 128]
    sim.particle_shape = 2  # B-spline order
    sim.space_charge = True
    sim.dynamic_size = True
    sim.poisson_solver = "fft"
    sim.prob_relative = [1.01, 1.01]
    ...
    energy_MeV = 1.000001 * mass_MeV  # reference energy (total)
    bunch_charge_C = 1.0e-9  # charge in C
    npart = int(10000000)  # number of macro particles
