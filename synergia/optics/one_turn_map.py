...
import numpy
from beamline import JetParticle, JetProton

from synergia.lattice import (
    chef_unit_conversion,
    get_chef_index,
)
...
def _convert_linear_map(chef_map, reference_particle):
    ...
    for row in range(0, 6):
        for column in range(0, 6):
            ...
            map[row, column] = chef_map.get(chef_row, chef_column) * u[row] / u[column]
    return map
...
def linear_one_turn_map(lattice_simulator):
    ...
    #                                                        map_order)
    lattice_simulator.get_chef_lattice().get_beamline().propagateJetParticle(
        jet_particle
    )
