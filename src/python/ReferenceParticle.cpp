...
void init_refparticle(py::module& m)
{
    py::class_<RefPart>(m, "RefPart")
        ...
        .def_readwrite("px", &RefPart::px, "momentum in x, normalized to proper velocity = beta_x*gamma [m*c]")
        .def_readwrite("py", &RefPart::py, "momentum in y, normalized to proper velocity = beta_y*gamma [m*c]")
        .def_readwrite("pz", &RefPart::pz, "momentum in z, normalized to proper velocity = beta_z*gamma [m*c]")
