...
def test_bunch_core_diagnostics(bunch_fixture2):
    dist = synergia.foundation.PCG_random_distribution(
        1234567, synergia.utils.Commxx.World.rank()
    )

    bunch_means = np.zeros(6)
    synergia.bunch.populate_6d(dist, bunch_fixture2, bunch_means, bunch_covariances)
